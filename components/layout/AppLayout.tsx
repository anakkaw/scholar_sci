...
export function AppLayout({ children, navItems, user }: AppLayoutProps) {
    ...
    const SidebarContent = () => (
        <div className="flex h-full flex-col">
            ...
            <div className="px-5 pt-5 pb-1">
                <p className="text-[10px] font-semibold tracking-widest text-amber-600/60 uppercase">เมนูหลัก</p>
            </div>
                        ...
                        return (
                            <Link
                                ...
                                className={cn(
                                    ...
                                    isActive
                                        ? "bg-gradient-to-r from-amber-100 to-amber-50 text-amber-900 font-semibold shadow-sm"
                                        : "text-amber-900/60 hover:bg-amber-50/80 hover:text-amber-800"
                                )}
                            >
                                {isActive && (
                                ...
                                <span className={cn(
                                    "transition-colors",
                                    isActive ? "text-amber-600" : "text-amber-400 group-hover:text-amber-600"
                                )}>
                                    {item.icon}
                                </span>
            ...
            <div className="px-5 py-2">
                <div className="relative h-px bg-amber-100">
            ...
            <div className="p-4 pb-3">
                <div className="relative flex items-center gap-3 rounded-xl bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-100 px-3 py-2.5 overflow-hidden">
                    {/* subtle pattern */}
                    ...
                    <Avatar className="h-8 w-8 border-2 border-amber-200 ring-2 ring-amber-100 flex-shrink-0 relative z-10">
                    ...
                    <div className="flex flex-col overflow-hidden relative z-10 flex-1 min-w-0">
                        <span className="text-xs font-semibold truncate text-amber-900">{user.name || "ผู้ใช้งาน"}</span>
            ...
            <div className="px-4 pb-4 text-center">
                ...
                <p className="text-[9px] text-amber-300/40 mt-0.5">v1.0.0</p>
            </div>
        </div>
    );
    ...
    return (
        <div className="grid min-h-screen w-full md:grid-cols-[220px_1fr] lg:grid-cols-[260px_1fr]">
            {/* Desktop Sidebar */}
            <div className="hidden border-r border-amber-100 bg-white md:block relative overflow-hidden">
            ...
            <div className="flex flex-col">
                {/* Top Header */}
                <header className="flex h-14 items-center gap-4 border-b border-amber-100/80 px-4 lg:h-[60px] lg:px-6 sticky top-0 z-30 bg-white/80 backdrop-blur-lg">
                    {/* Mobile menu button */}
                    <Sheet open={open} onOpenChange={setOpen}>
                        <SheetTrigger asChild>
                            <Button
                                variant="outline"
                                size="icon"
                                className="shrink-0 md:hidden border-amber-200 hover:bg-amber-50 rounded-xl"
                            >
                                <Menu className="h-4 w-4" />
                                <span className="sr-only">Toggle navigation menu</span>
                            </Button>
                        </SheetTrigger>
                        <SheetContent side="left" className="flex flex-col p-0 border-amber-100">
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="ghost" size="icon" className="rounded-full ring-2 ring-amber-100 hover:ring-amber-200 transition-all">
                        ...
                        <DropdownMenuContent align="end" className="w-56 rounded-xl border-amber-100 shadow-lg">
                            ...
                            <DropdownMenuItem asChild className="rounded-lg focus:bg-amber-50 focus:text-amber-900 cursor-pointer">
                            ...
                            <DropdownMenuSeparator className="bg-amber-50" />
                            <DropdownMenuItem
                                className="rounded-lg text-red-600 focus:bg-red-50 focus:text-red-700 cursor-pointer"
                                onClick={() => signOut()}
                            >
                ...
                {/* Main Content */}
                <main className="flex flex-1 flex-col gap-4 p-4 lg:gap-6 lg:p-6 bg-gradient-to-br from-amber-50/30 via-white to-orange-50/20 min-h-0">
                    {children}
                </main>
            </div>
        </div>
